---
name: cto-engineering-org-design
description: Design an engineering organisation using Team Topologies. Outputs stream-aligned/enabling/platform/complicated-subsystem teams, spans of control, staffing model, and 90-day rollout.
argument-hint: [current org, product streams, size target, cognitive load hotspots]
allowed-tools: Read, Write
audience: CTO
---

# Engineering Org Design & Topology

A decision-grade playbook for the **CTO** and executive peers. Use it to move from ambiguous business context to a clear, defensible narrative that a board, exec committee, or investment committee can act on within one working session.
...
**Context.** A £2B financial services group with a fragmented data estate, six analytical platforms, and rising regulatory demand under DORA and Consumer Duty. The CTO has been asked to bring a decision to the December board.
...
**Governance.** Monthly steering chaired by the CTO, quarterly board update, independent assurance review at each wave exit.
