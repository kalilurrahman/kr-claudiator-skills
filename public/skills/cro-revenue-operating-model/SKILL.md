---
name: cro-revenue-operating-model
description: Redesign the revenue engine — segments, coverage, quotas, comp, forecasting, and RevOps stack. Outputs coverage model, quota-capacity plan, forecast cadence, and pipeline hygiene guardrails.
argument-hint: [TAM, current pipeline, sales org, deal profile, forecast accuracy]
allowed-tools: Read, Write
audience: CRO
---

# CRO Revenue Operating Model

A decision-grade playbook for the **CRO** and executive peers. Use it to move from ambiguous business context to a clear, defensible narrative that a board, exec committee, or investment committee can act on within one working session.
...
- Framing a major bet where capital, org change, and risk trade off against each other.
...
**Context.** A mid-cap enterprise where the CRO has been asked to bring a decision to the next board meeting on the topic described in the argument hint.

**Recommendation.** Proceed in three sequenced waves with a hard kill-switch at Wave 1 exit if unit economics miss target by more than 15%.

**Value.** Base case value in the tens of millions over 3-5 years, with an explicit range and two dominant assumptions.

**Ask.** Capital envelope, peak FTE, board decision date, first value milestone date.

**Kill switch.** If Wave 1 does not deliver the target leading indicators by month 9, the programme is paused and re-scoped before further capital is released.

**Top risks.** Talent scarcity, data-quality debt, change fatigue, regulatory drift, vendor concentration — each with named owner and mitigation.

**Governance.** Monthly steering chaired by the CRO, quarterly board update, independent assurance review at each wave exit.
...
- ceo-quarterly-business-review — to fold into the QBR narrative.
...
- cxo-crisis-communication-playbook — when execution risk needs a comms plan.
