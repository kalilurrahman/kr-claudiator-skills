...
description: Plan a tiered launch (alpha → GA) with success metrics and rollback gates.
argument-hint: [feature, audience tiers, gates]
...
The canonical workflow for **Launch Playbook** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Plan a tiered launch (alpha → GA) with success metrics and rollback gates.
>
