...
description: Specify tool contracts, schemas, and guardrails for an LLM agent.
argument-hint: [agent goal, tool set, risk class]
...
The canonical workflow for **Agent Tool Design** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Specify tool contracts, schemas, and guardrails for an LLM agent.
>
