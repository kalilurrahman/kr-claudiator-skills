...
description: Bootstrap an ArgoCD/Flux GitOps environment with sync waves and drift detection.
argument-hint: [tool, cluster count, app inventory]
...
## When to use
Bootstrap an ArgoCD/Flux GitOps environment with sync waves and drift detection.
...
The canonical workflow for **Gitops Bootstrap** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Bootstrap an ArgoCD/Flux GitOps environment with sync waves and drift detection.
>
