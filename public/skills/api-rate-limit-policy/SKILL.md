...
description: Define per-route, per-tenant rate limits with burst, throttling, and 429 contracts.
argument-hint: [traffic mix, tenants, slo]
...
## When to use
Define per-route, per-tenant rate limits with burst, throttling, and 429 contracts.
...
The canonical workflow for **Api Rate Limit Policy** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Define per-route, per-tenant rate limits with burst, throttling, and 429 contracts.
>
