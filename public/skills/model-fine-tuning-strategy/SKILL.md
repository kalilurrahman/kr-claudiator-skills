...
The canonical workflow for **Model Fine Tuning Strategy** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Plan and execute model fine-tuning to improve performance on domain-specific tasks. Outputs fine-tuning decision framework, dataset preparation guidelines, eval
>
