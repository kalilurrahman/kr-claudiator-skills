---
name: dependency-upgrade-playbook
description: Triage and upgrade transitive dependencies safely with lockfile diffs and CVE checks.
argument-hint: [stack, cadence, blast radius]
...
## When to use
Triage and upgrade transitive dependencies safely with lockfile diffs and CVE checks.
...
The canonical workflow for **Dependency Upgrade Playbook** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Triage and upgrade transitive dependencies safely with lockfile diffs and CVE checks.
>
