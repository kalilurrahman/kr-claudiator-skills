...
description: Right-size workloads, enable HPA/VPA, and apply scheduling policies to cut spend.
argument-hint: [cluster size, workload mix, budget target]
...
## When to use
Right-size workloads, enable HPA/VPA, and apply scheduling policies to cut spend.
...
The canonical workflow for **Kubernetes Cost Optimization** runs in five stages. Each stage has an explicit exit criterion so the skill knows when to advance.
...
> **Context:** Right-size workloads, enable HPA/VPA, and apply scheduling policies to cut spend.
>
